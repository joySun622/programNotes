[toc]

# 学习大纲

1. [来源网站](https://www.tulingxueyuan.cn/wp-content/uploads/2021/04/kechengdagang-1.png)

![img](images/e517a7d5de0741f51b8caffaac73af18.png)

# 查询国内DNS

- [中国站长工具]( http://tool.chinaz.com/dns/)
  查看DNS，测速等

# 星环科技

- [星环科技说明文档](https://www.warpcloud.cn/#/documents-support/docs?category=TDH)

- [星环科技Inceptor使用手册](https://www.warpcloud.cn/#/documents-support/docs-detail/document/TDH-PLATFORM/9.0/010InceptorManual?docType=docs&docName=Inceptor%E4%BD%BF%E7%94%A8%E6%89%8B%E5%86%8C)

# 接口管理&设计&测试工具

- [Eolink](https://www.eolink.com/)
- [Apifox](https://www.apifox.cn/)

# 连接Linux工具

[XFTP](https://www.xshell.com/zh/free-for-home-school/)

[XSHELL](https://www.xshell.com/zh/free-for-home-school/)

# 浏览器兼容性查询

> 查询一些`js`函数或者标签在哪些浏览器中可以兼容。

1. [浏览器兼容性查询网址Caniuse](https://caniuse.com/)

# NPM国内镜像网址

- 当前 [npmmirror.com](https://npmmirror.com/) 运行版本是: cnpmcore@1.0.0-rc.1
- [NPM官网](https://www.npmjs.com/):可搜索一些使用npm的插件
- 本系统运行在 [Node.js](https://nodejs.org/)@v16.13.2 上
- 开源镜像: https://npmmirror.com/mirrors/
- Node.js 镜像: https://npmmirror.com/mirrors/node/
- alinode 镜像: https://npmmirror.com/mirrors/alinode/
- ChromeDriver 镜像: https://npmmirror.com/mirrors/chromedriver/
- OperaDriver 镜像: https://npmmirror.com/mirrors/operadriver/
- Selenium 镜像: https://npmmirror.com/mirrors/selenium/
- electron 镜像: https://npmmirror.com/mirrors/electron/
- 

# JAVA

## JDK下载网址

[各版本源码下载])(http://hg.openjdk.java.net/)

# API文档框架

## Swagger

[Swagger官网](https://swagger.io/)

## Ali RAP2

[Ali RAP2](http://rap2.taobao.org/):可进行WebAPI接口的数据模拟&测试

## Apifox

- [Apifox 官方网址](https://www.apifox.cn/)

> 可进行接口数据的模拟&测试，有线上和桌面应用

# 前端知识

## ECMAScript6

1. [ES6文档说明](http://es6.ruanyifeng.com/)

# 前端开发工具

## VS CODE

1. [VS CODE 官网](https://code.visualstudio.com/)
2. [VS_CODE官方说明文档](https://code.visualstudio.com/docs)

# 前端显示插件

## FeHelper

[GitHub下载网址](https://github.com/zxlie/FeHelper)

> 本插件支持**Chrome、Firefox、MS-Edge**浏览器，内部工具集持续增加，目前包括 JSON自动/手动格式化、JSON内容比对、代码美化与压缩、信息编解码转换、二维码生成与解码、图片Base64编解码转换、Markdown、 网页油猴、网页取色器、脑图(Xmind)等贴心工具，甚至在目前新版本的FeHelper中，还集成了**FH开发者工具**

# 前端插件

## 图表插件

### Echarts

- [Echarts官网](https://echarts.apache.org/zh/index.html)

### AntV

- [AntV官方网址](https://antv.vision/zh)

### HighCharts

- [HighCharts官网](https://www.highcharts.com/demo/maps)

### D3

- [D3官网](https://d3js.org/)

## 地图插件

### 高德地图

- [高德地图API](https://developer.amap.com/api/jsapi-v2/summary/)

### 百度地图

- [百度地图API](https://lbsyun.baidu.com/index.php?title=jspopular3.0)

## GEO JSON自定义

- [GEO JSON](http://geojson.io/#map=2/20.0/0.0)
- [DataV.GeoAtlas](http://datav.aliyun.com/portal/school/atlas/area_selector#&lat=30.332329214580188&lng=106.72278672066881&zoom=3.5)

# 数据库

## [MongoDB官网](https://docs.mongodb.com/)

# 搜索引擎

## ES

[中文官网](https://www.elastic.co/cn/elasticsearch/)

[Git Hub开源网址](https://github.com/elastic/elasticsearch)

# 应用排行榜

## 数据库排行榜

1. [db-engines 排行榜](https://db-engines.com/en/ranking)

# JVM调优工具

## Arthas

> [官方文档](https://arthas.aliyun.com/doc/)

# 数据结构

>  [Data Structure Visualizations](https://www.cs.usfca.edu/~galles/visualization/Algorithms.html)

# Maven

- [Maven中央库网址](https://search.maven.org/)
  可查看相关依赖版本

# MyBatisPlus

[MyBatisPlus说明文档](http://mp.baomidou.com/guide/quick-start.html)

# SpringCloud

> 1. [SpringCloud官网](https://spring.io/projects/spring-cloud)
> 2. [Ribbon文档](https://github.com/Netflix/ribbon/wiki/Getting-Started)
> 3. [Hytrix文档](https://github.com/Netflix/Hystrix/wiki/How-To-Use)

# Spring官网

> https://spring.io/

# SpringBoot

> 1. [官方最新参考指南文档](https://docs.spring.io/spring-boot/docs/current/reference/html/)  
> 2. Spring Boot 参考指南可用如下方式：
>    - [HTML](https://docs.spring.io/spring-boot/docs/2.0.2.RELEASE/reference/html/)
>    - [PDF](https://docs.spring.io/spring-boot/docs/2.0.2.RELEASE/reference/pdf/spring-boot-reference.pdf)
>    - [EPUB](https://docs.spring.io/spring-boot/docs/2.0.2.RELEASE/reference/epub/spring-boot-reference.epub)

# 国内镜像源

1. 清华[TUNA](https://mirrors.tuna.tsinghua.edu.cn/)镜像源：https://mirrors.tuna.tsinghua.edu.cn/
   https://pypi.tuna.tsinghua.edu.cn/simple

2. 中科大[USTC](https://mirrors.ustc.edu.cn/)镜像源：  https://mirrors.ustc.edu.cn/  
   https://pypi.mirrors.ustc.edu.cn/simple/

3. 网易163： http://mirrors.163.com/

4. [阿里](https://opsx.alibaba.com/mirror  )： https://opsx.alibaba.com/mirror 
   http://mirrors.aliyun.com/pypi/simple/
5. 华为：https://mirrors.huaweicloud.com/
6. 腾讯：https://mirrors.cloud.tencent.com/
7. 北京交通大学：https://mirror.bjtu.edu.cn/
8. 北京理工大学：http://mirror.bit.edu.cn/web/
9. 华中科技大学：http://mirrors.hust.edu.cn/
10. 兰州大学：http://mirror.lzu.edu.cn/
11. 谷歌学术镜像：https://ac.scmor.com/



# Git

1. [git 官方文档](https://git-scm.com/book/en/v2)



# 工作流框架

1. [Activit](https://www.activiti.org)



# CSS

1. [CSS帮助文档](http://www.w3.org/TR/CSS21/colors.html#propdef-background-position)

   

# 网站IP查询

1. [ipaddress](https://www.ipaddress.com/ip-lookup)
2. [IP查询网](https://site.ip138.com/)

# 几何画图工具

1. [Geogebra](https://www.geogebra.org/)
2. [AxGlyph ](https://www.amyxun.com/)
3. [desmos](https://www.desmos.com/?lang=zh-CN)
4. [netpad](https://www.netpad.net.cn/)

# 表格数据格式转换

1. [tableconvert](https://tableconvert.com/):在线转换工具，可将excel表格数据指定文本格式。如将excel数据转换为markdown格式

# 可视化报表

- [FineReport ](www.finereport.com)
  一个报表软件，企业级的应用。用于系统的开发业务报表，数据分析报表。也可集成在OA，ERP，CRM等应用系统内，做数据报表模块，也可以开发成财务分析系统

# 商业智能分析软件

- [Tableau ](www.tableau.com)
- [FineBI](www.finebi.com)

# 高校学习网站

1. [中国大学慕课](http://www.icourse163.org )
2. [学堂在线](http://www.xuetangx.com/ )
3. [MOOC](https://www.icourse163.org/)

# 留学

1. [启德考培在线]( http://qide.edusoho.cn/)

# 职业考试

1. [中公教育](http://www.offcn.com/)

# 职业技能

1. [51cto]( http://edu.51cto.com/)

# 视频软件

1. [福昕视频压缩大师](www.foxitsoftware.cn/shipinyasuo)
2. [HandBrake](https://handbrake.fr/rotation.php?file=HandBrake-1.5.1-x86_64-Win_GUI.exe)

# 视频HTML5插件

- [Polyv HTML5 Player旧官网](http://player.polyv.net/script/index.html)
- [polyv 保利威官方文档](https://dev.polyv.net/2017/videoproduct/v-player-sdk/v-playerapi/html5player/html5-docs/)

# 在线画图链接

> PS可以使用WPS建立思维导图，但一般用户也有节点限制

- [Mind Line](http://www.mindline.cn/webapp)

- [Board Mix](https://boardmix.cn/)  无节点限制，功能较丰富；拥有电脑客户端

- [ProcessOn](https://www.processon.com/)

- [爱莫](https://mind.airmore.cn/)

  > 建立导图时，有节点限制

# 电子书阅读电脑版

- [calibre](https://calibre-ebook.com/)

  > 开源免费电子书管理器,阅读版面有点糟糕，无法自动提取目录。如果想要目录的话，需自己设置。
  >
  > 【转换书籍】-【结构检测】
  >
  > ```
  > //*[re:test(., "^\s*[第卷][0123456789一二三四五六七八九十零〇百千两]*[章回部节集卷].*", "i")]
  > ```
  >
  > 进行转换后，才能生成目录。打开文本的速度有点慢

- neat reader

  > 可自动提取目录。付费软件，但很多设置如字体，背景色等设置需vip



# 网站素材

## 背景图

1. [熊猫办公](https://www.tukuppt.com/backdrop/zonghe_0_0_0_159_0_0_3.html)
2. [觅知网](https://www.51miz.com/so-dianshang/85001.html)
3. [千图](https://www.58pic.com/newpic/38711274.html)
4. [红动中国](https://so.redocn.com/UI/7569cafdbeddbde7c3e6.htm)
5. [17素材](https://www.17sucai.com/search/%E8%A1%A8%E6%A0%BC?)

# 注册市场监管局账号

- [12315平台](https://www.12315.cn/cuser/index)

# 图文识别工具

- 在线识别：https://web.baimiaoapp.com/

# 参考来源

1. https://zhuanlan.zhihu.com/p/113554606
2. https://tableconvert.com/

