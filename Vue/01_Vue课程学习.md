...
>
>   ```
>   import navbar from '@/components/Navbar'  //@ 别名===>指向src的绝对路径
>   
...
> 解决跨域问题:因为端口号和域名差异，导致的跨域问题。服务器与服务器是没有跨域限制的，但是浏览器有跨域限制。在vue中，通过反向代理解决跨域问题
...
```
module.exports = defineConfig({
  lintOnSave: false,//暂时关闭lint
  //配置代理
  devServer: {
    // disableHostCheck:true,
    host: '0.0.0.0',//配置IP，默认为localhost
    port: 8080,//配置当前前端服务端口
    proxy: { //配置代理；可以根据需要配置多个代理
      '/api': {
        target: 'http://127.0.0.1:4523/m1/1357016-0-default', // API服务器的地址
        ws: true, // 代理websockets
        changeOrigin: true, // 是否跨域，虚拟的站点需要更新origin
        pathRewrite: { //路径重写
...
> 1. [vue router官方网址](https://router.vuejs.org/)
> 2. **路由原理**:Vue router组件，封装了路由切换&监听操作，可以方便的管理路由切换
...
import WaybillList from '../views/WaybillList.vue' //引入模块

Vue.use(VueRouter)  //注册路由插件；俩个全局组件router-view  router-link

const routes = [
  {
    path: '/', //访问路径
    name: 'WaybillList',
    component: WaybillList //组件
  }
]
...
> 1. 可以直接使用`<router-view></router-view>`，在页面中引入其他组件页面，如在`App.vue`中使用，直接在首页展示路由配置中的组件页面
> 2. 可以使用`<router-link to="/" tag="a" class="router-link-item">异常单号</router-link>`引入组件页面，通过配置路径，引入对应的页面
...
import Vue from 'vue'
import VueRouter from 'vue-router'
import films from '../views/Films.vue' //引入模块
const routes = [
  {
    path: '/films', //访问路径
    name: 'films',
  ...
  {
    path: '/', //访问路径
    redirect: '/films',//重定向到/films路径,当访问路径为/会被重定向到路径/films
  },
  {
    path: '*', //访问路径
    redirect: '/films',//重定向到/films路径：任意访问路径都会被重定向到路径/films
  },
]
```
...
> 如通过`a`链接的方式，进行跳转，称为声明式导航。还有一种编程式导航，是通过js进行页面跳转的方式。

```
<router-link :to="...">  声明式路由导航
属性：
to  设置路由路径
active-class  设置当前路径class名称，可根据设置的class名定义当前访问路径的样式；
tag  设置路由渲染的节点类型 ，若不设置，默认为a标签。vue router 4后不再支持，使用会有警告提示
```

- **范例**

```
### vue router 4版本后，连接定义如下
<router-link to="/films" custom v-slot="{navigate,isActive}">
  <li @click ="navigate" :class="isActive?'color1':'color2'">电影</li>
</router-link>
```

## 嵌套路由

> 一些应用程序的 UI 由多层嵌套的组件组成。如下：
>
> ```
...
> ```
>
> 如果要实现，在User界面，显示指定人员的/johnny/profile页面，即User的子结构。在定义路由的时候，需定义嵌套结构，如果是是平级结构，会默认将子结构信息页面替换父页面，不满足需求
...
<template>
  <div>
    <h1>User</h1>
    <router-view></router-view>  <!--会根据访问路径渲染页面：如当访问路径为/user/:id/posts时，显示posts信息，-->
  </div>
</template>
```
...
> 在列表信息页面，点击单条记录，进入该记录详情页面；点击返回，可以返回列表页面。
...
<template>
  <div>
     <router-link to="/detail">11111</router-link>
  </div>
</template>
...
<template>
  <div>
     <a @click="handlerClick">11111</a>
  </div>
</template>

 methods: {
   handlerClick(){
      location.href="#/detail" //老写法，存在路径不是 #/detail情况
      this.$router.push('/detail');//路由写法
   }
 }
```
...
```
const User = {
  template: '<div>User {{ this.$route.params.id }}</div>', //获取路径中的ID值
}
```
...
除了 `this.$route.params` 之外，`this.$route` 对象还公开了其他有用的信息，如 `this.$route.query`（如果 URL 中存在参数）、`this.$route.hash` 等。你可以在 [API 参考](https://router.vuejs.org/zh/api/#routelocationnormalized)中查看完整的细节。

## 命名路由

> 除了 `path` 之外，你还可以为任何路由提供 `name`；通过路由名作为该路由标识。
...
- 使用方式1：要链接到一个命名的路由，可以向 `router-link` 组件的 `to` 属性传递一个对象：
...
> `hash`模式&`history`模式区别
>
> 1. 在浏览器路径显示上的区别
>
>    **hash** :`http://localhost:8080/#/statistic ` 显示有点欠缺，在某些场景下可能会报错
>    **history**: `http://localhost:8080/statistic`  显示更好看不过，问题来了。
>    由于我们的应用是一个单页的客户端应用，如果没有适当的服务器配置，用户在浏览器中直接访问 `https://example.com/user/id`，就会得到一个 404 错误。这就丑了。
>
...
> 一个应用页面，点击任何操作，需验证用户是否登录；若未登录，跳转到登录页面。
...
router.beforeEach((to, from) => {
  ...
  if (to.meta.requiresAuth && !auth.isLoggedIn()) {
    ...
    return {
      path: '/login',
      // 保存我们所在的位置，以便以后再来；redirect可以为任意字符
...
const UserDetails = {
  template: `...`,
  beforeRouteEnter(to, from,next) {//next可选
    // 在渲染该组件的对应路由被验证前调用
    // 不能获取组件实例 `this` ！
    // 因为当守卫执行时，组件实例还没被创建！
  },
...
7. https://router.vuejs.org/zh/guide/essentials/dynamic-matching.html
